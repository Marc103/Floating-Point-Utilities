...
I am currently part of the BiV lab headed by Professor Emma Alexander at Northwestern University (see https://www.alexander.vision/). DfDD in general is a technique where if you capture two images of the same scene but with slightly different focus levels, you can use those defocus cues to extract depth information (to produce a sparse depth map). As you can see in the setup, we have to two image sensors recieving the same image from the beam splitter, but at slightly different focus levels, and feeding them to the development board via the ribbon cables. In order to align the images, we first used bilinear interpolation. Then through a series of convolutions and finally a division, it produces a depth map which is then communicated via an FT232H chip. This is all pipelined and accomplished via stream processing which allows us to buffer way fewer lines, meaning we don't need dedicated SDRAM which would consume a lot of power and can instead rely on the FPGA's embedded memory. I developed the RTL code to perform all these tasks on the FPGA (ECP5). We submitted a paper to CVPR which i was co-first authored with Tianao Li and John Mamish. See the conceptual diagram below and link to live demo here: https://youtu.be/L_AHa2lpS0A 

![General_Overview_shaded_5_jpg](https://github.com/user-attachments/assets/11619465-be96-4957-a5d9-621f77bce687)
